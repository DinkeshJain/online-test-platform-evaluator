...
const InternalMarksEvaluation = () => {
  ...
  const [tempClearedStatus, setTempClearedStatus] = useState({}); // Track temporarily cleared marks
  const saveTimeoutRef = useRef({});
  const clearedTimeoutRef = useRef({});
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      Object.values(clearedTimeoutRef.current).forEach(timeout => {
        if (timeout) clearTimeout(timeout);
      });
    };
  ...
  const handleMarksChange = (studentId, value) => {
    ...
    // Trigger autosave for both values and empty fields (clearing marks)
    if (value !== '') {
      debouncedSave(studentId, numValue);
    } else {
      // Handle mark clearing - send empty value to backend
      debouncedSave(studentId, '');
    }
  };
  ...
  const saveInternalMarks = async (studentId, marksValue) => {
    try {
      ...
      const requestData = {
        ...
        internalMarks: marksValue === '' ? null : marksValue, // Send null for empty values to trigger deletion
        isMarkBeingCleared: marksValue === '' || marksValue === null || marksValue === undefined
      };
      ...
      // For cleared marks, set savedMarks to null to indicate no saved value
      setSavedMarks(prev => ({
        ...prev,
        [studentId]: marksValue === '' ? null : marksValue
      }));

      // If marks were cleared, show temporary cleared status for 2 seconds
      if (marksValue === '' || marksValue === null || marksValue === undefined) {
        setTempClearedStatus(prev => ({
          ...prev,
          [studentId]: true
        }));

        // Clear the temporary cleared status after 2 seconds
        if (clearedTimeoutRef.current[studentId]) {
          clearTimeout(clearedTimeoutRef.current[studentId]);
        }
        
        clearedTimeoutRef.current[studentId] = setTimeout(() => {
          setTempClearedStatus(prev => ({
            ...prev,
            [studentId]: false
          }));
      ...
      // Clear temporary saved status after 3 seconds (but keep the green color for non-cleared marks)
      setTimeout(() => {
        setSaveStatus(prev => ({
          ...prev,
          [studentId]: ''
  ...
  const getInputStyling = (studentId) => {
    ...
    // Handle both empty values (cleared marks) and numeric values
    if ((currentValue === '' && savedValue === null) || 
        (currentValue !== '' && currentValue == savedValue && savedValue !== null)) {
                    ...
                    <tr key={student._id} className="border-b hover:bg-gray-50">
                      ...
                      <td className="p-4">
                        <div className="flex items-center gap-1">
                          ...
                          {saveStatus[student._id] === 'saved' && (
                            <>
                              ...
                              <span className="text-sm text-green-600 font-medium">
                                {tempClearedStatus[student._id] ? 'Cleared' : 'Saved'}
                              </span>
                            </>
                          )}
                          ...
                          {!saveStatus[student._id] && (
                            <>
                              {marks[student._id] !== '' && savedMarks[student._id] !== null && marks[student._id] == savedMarks[student._id] && (
                                <>
                                  <CheckCircle className="h-4 w-4 text-green-500" />
                                  <span className="text-sm text-green-600 font-medium">Saved</span>
                                </>
                              )}
                              {/* Only show "Cleared" temporarily when tempClearedStatus is true */}
                              {tempClearedStatus[student._id] && marks[student._id] === '' && savedMarks[student._id] === null && (
                                <>
                                  <CheckCircle className="h-4 w-4 text-green-500" />
                                  <span className="text-sm text-green-600 font-medium">Cleared</span>
                                </>
                              )}
                              {!marks[student._id] && savedMarks[student._id] === undefined && (
                                <span className="text-sm text-gray-400">No marks</span>
                              )}
                              {marks[student._id] && savedMarks[student._id] !== null && marks[student._id] != savedMarks[student._id] && (
